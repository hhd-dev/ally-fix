...
def exit(code=0):
    if code != 0:
        ...
        print(f"Open an issue here to diagnose: https://github.com/hhd-dev/ally-fix")
...
try:
    ...
    for v in enumerate(vid=ASUS_VID):
        ...
        usage = v["usage"]
        print(
            f"Found Ally device with usage_page 0x{usage_page:04x} and usage 0x{usage:04x}."
        )
    ...
    if not dev:
        ...
        for cmd in cmds:
            print(f"C: {cmd.hex()}")
            dev.write(cmd)
        print(f"Switched to rainbow mode.")
        print(f"Fix applied successfully.")
        exit()
except Exception as e:
    import traceback

    print(f"The following error occurred and prevented the fix from running:")
